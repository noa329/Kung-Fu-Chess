...
> **Status: I1, I2, and I3 done.** `GameStateSerializer` emits the sparse
> `activeMoves` array (I1), `GameStateDeserializer` parses it back into
> `GameSnapshot::moveTargets`/`moveProgress` (I2), and a real-socket test
> against a live `WebSocketServer` (I3) proves both ends against the
> actual wire — not just the pure serializer/deserializer unit tests.
> Pick up at **I4** (client wiring/verification) when resuming.
