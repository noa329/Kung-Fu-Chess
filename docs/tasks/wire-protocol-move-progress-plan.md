...
> **Status: I1 through I5 all done — the whole plan is complete.**
> `GameStateSerializer` emits the sparse `activeMoves` array (I1),
> `GameStateDeserializer` parses it back into
...
> (I3), a scripted `OnlineClient`-driven check confirmed real, climbing
> `moveProgress` values reach the networked graphics client's own
> `GameSnapshot` with zero client-side code changes (I4's scripted half),
> and the user's own manual click-through confirmed pieces visibly slide
> smoothly in Online Play, matching Local Play exactly (I4's GUI half).
> `graphics-networked-client-plan.md`'s own Decision 1 is updated to point
> here as its resolution (I5). This closes out the H+I combined plan —
> Online Play now has real slide animation, sourced from the same wire
> data the sparse bandwidth design (Decision 1 above) was built around.
...
| **I4** ✅ | Wire verification on the graphics client — no client-side code changed, per Decision 4. **Scripted half**: a throwaway (not committed) console program linking `OnlineClient` directly (`kungfu-graphics/cpp/src/`, no OpenCV needed — same no-GUI approach H4–H7 used), driven through a temporary CMake target added to `kungfu-graphics/cpp/CMakeLists.txt` for the build, then reverted (`git checkout --`) once verified. **Real finding along the way**: the very first tick reporting `cellStates[6][4] == "move"` can legitimately show `moveProgress == 0.0` (the scheduling tick itself) — a single-sample "0 < progress < 1" assertion is flaky by construction, not a bug in the product code. Fixed by sampling repeatedly over a real window and asserting progress actually climbs (`0.000 → 0.303` in the passing run), which is what `BoardView::render()`'s per-frame interpolation actually depends on. **GUI half**: the user's own manual click-through, over a real two-instance session — confirmed pieces slide smoothly in Online Play, matching Local Play. | I2, H5 | Scripted program: logged in, created a room alone (immediate starting-position broadcast, confirmed idle at e2), sent `WPe2e4`, sampled `pollGameState()` until `moveProgress[6][4]` climbed past 0.3 while `moveTargets[6][4]` correctly read `(4,4)` throughout, then confirmed the move resolved (`board[4][4] == "wP"`). All 4 checks passed against a freshly-launched `kungfu_server.exe`. `KungFuChess` (MSVC/CMake) reconfigured + rebuilt clean after the scratch target/CMake edit/source file were fully removed, confirming zero residue. Manual click-through: user-confirmed, real GUI session. |
| **I5** ✅ | Docs closeout. Updated `graphics-networked-client-plan.md`'s own Decision 1 (which explicitly deferred this) to point at this plan as its resolution — `moveProgress`/`moveTargets` are now resolved, `captureFlashes` stays explicitly out of scope/deferred. **README.md turned out to need no change**: re-read H8's actual committed diff before editing anything and confirmed the "pieces snap between ticks" framing this row expected to remove was never actually written into README.md in the first place (H8's Online Play section describes controls/flow generically, not animation fidelity) — so there was nothing stale to correct there. Documenting this rather than inventing an edit, per the "every claim re-verified against the actual behavior/file before being written down" bar this whole plan (and H8 before it) has held to throughout. | I1–I4 | N/A — documentation only. |
...
Nothing to resume — I1 through I5 are all done. `captureFlashes` remains
explicitly out of scope (see above); a future plan revisiting it would be
a new, separate document, not a reopening of this one.
