...
> **Status: I1 and I2 done.** `GameStateSerializer` emits the sparse
> `activeMoves` array (I1) and `GameStateDeserializer` parses it back into
> `GameSnapshot::moveTargets`/`moveProgress` (I2) — real values now reach
> the networked graphics client instead of the old unconditional sentinel
> default. Pick up at **I3** (real-socket verification) when resuming.
