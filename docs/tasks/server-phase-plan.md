...
| **G3** | Voluntary `resign` wire command, reusing E2's sender-identity model instead of a claimed-color field: a new literal command string `"resign"` (unambiguous against `GameCommandParser`'s strict-uppercase move/jump grammar), intercepted in `WebSocketServer::onMessage` before `GameCommandParser::parse`, routed to a new `GameSession::handleResign(username)` (same `ERROR NOT_A_PLAYER` rejection as `handleCommand` for a spectator/unjoined username; otherwise `engine().resign(colorOf(username))` - `GameEngine::resign()` already no-ops if `gameOver`, so a stray double-resign is safe for free, no new guard needed). No client-side parsing needed (`client/cli`'s gameplay loop already forwards any typed line verbatim, Task B4's stance) - just a prompt-text hint. Logging is automatic too - `GameSession` already subscribes `Logger` to `onGameLifecycle` (A6), so a voluntary resign gets the same `lifecycle phase=end` line a king capture or auto-resign gets, no new log call. **Resolved open question: cancel the pending 20s disconnect timer (if any) for the session when a resign is handled**, rather than accepting it as a cosmetic leftover - small cost, avoids `whiteDisconnectMs`/`blackDisconnectMs` still counting down in the broadcast for a few ticks after `gameOver` is already true. | A3, D4, E2 | doctest: `handleResign` ends the game in favor of the other color both ways, fires the lifecycle event, rejects a spectator/unjoined username without mutating engine state, no-ops on an already-`gameOver` session; a case with a live disconnect timer confirms it's cancelled by a resign (no further countdown ticks broadcast after). Manual: real two-`kungfu_client.exe` resign exchange, `server.log` corroborated. `kungfu_server_tests` (G2) gains a resign scenario alongside its existing four. |
