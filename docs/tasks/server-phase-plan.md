...
| **G1** | Root `README.md` - none existed before this, build/run instructions for all four binaries lived only as prose scattered across `CLAUDE.md`, `kungfu-graphics/cpp/README.md`, and this file's own manual-verification sections. Mostly synthesis, not new research: `run_tests.exe` (`make && ./run_tests.exe`), the graphics `KungFuChess.exe` (links to `kungfu-graphics/cpp/README.md` for the `OpenCV_451` download, then the CMake/VS Code steps), `kungfu_server.exe`/`kungfu_client.exe` (MSYS2/ucrt64, `cmake -S server -B server/build -G Ninja` + `cmake --build server/build`, run from the repo root), plus a pointer to `scripts/build_vpl_submission.sh` for the course grader zip and a link into `docs/tasks/` for the full design history. Deliberately last in Phase G so it documents G2/G3/G4's real final state (including `ctest`, `resign`, and the new discrete event messages) rather than needing an edit right after landing. | G2, G3, G4 (documents their end state) | N/A - documentation only. |
