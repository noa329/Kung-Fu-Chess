...
| **G4** | Push two of the four `EventBus` categories to clients as discrete messages instead of only being inferable a tick later from the periodic snapshot - `onSound` (today reaches **no** networked client at all; A4 deliberately excluded sound from `GameStateSerializer`, same bucket as `captureFlashes`) and `onGameLifecycle` (start/end - meaningful one-shot cues, and this is what makes G3's resign show up immediately rather than only via `gameOver` flipping on the next tick). **Resolved open question: `onSound` + `onGameLifecycle` only, not `onMoveLogged`/`onScoreUpdated`** - both of those are already reflected within one tick via the existing snapshot broadcast, so a discrete push there would shave latency but add no new capability; scope stays to what's genuinely missing today. Same optional-attach pattern as `attachLogger()`: new `GameSession::attachEventSink(std::function<void(const std::string&)>)`, subscribed to `onSound`/`onGameLifecycle` at construction (gated non-null, mirrors A6's stance so every existing default-constructed `GameSession()` in tests stays unaffected), publishing a small pre-serialized JSON string (`{"type":"sound","name":"capture"}`, `{"type":"lifecycle","phase":"end","result":"White Wins"}`). `WebSocketServer::createSession()` wires the sink to a lambda broadcasting to every connection currently in that session (`sessionManager_.connectionsIn(sessionId)`, the same loop `broadcastState()` already uses). Safe under the confirmed single-threaded `io_context` concurrency model - publishing happens synchronously on the same thread `broadcastState()` already runs on, no new thread-safety surface. | A6, D4 (announceStart/resign both need to reach clients promptly) | doctest: a capture publishes exactly one `{"type":"sound","name":"capture"}` sink message; `announceStart()`/`resign()` each publish exactly one `{"type":"lifecycle",...}` message with the right phase/result; no sink attached stays silent (mirrors A6's `Logger*` test). `kungfu_server_tests` (G2) gains a scenario asserting a real client receives a discrete `"sound"` message on a real capture, not just an eventual snapshot-field change. |
