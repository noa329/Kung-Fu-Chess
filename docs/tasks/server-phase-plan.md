...
| **E2** ✅ | Sender-identity-based command authorization (not just spectator enforcement - the flagged complexity from E1 turned out to be the real scope: no connection's claimed color was ever checked against who actually holds it, so a player could impersonate their opponent too, not only a spectator impersonating a player). `GameSession::handleCommand(username, cmd)` (signature change - `username` added, resolved by `WebSocketServer` from the sending `hdl` via the existing `authenticatedUsers_` map, same as every other username lookup since D4) now checks `colorOf(username)` - the seat this specific connection actually holds, fixed once at `join()`, never re-derived from the command text - against `cmd.color` *before* any board-state check: an unjoined/spectator username is rejected outright (`ERROR NOT_A_PLAYER`), and a real player claiming the *other* color is rejected too (`ERROR NOT_YOUR_COLOR`) - this second case is the one the old board-only check could never catch, since a claimed color that's also genuinely on the board (e.g. Black's connection claiming White's own real rook) would have sailed straight through. Confirmed by a doctest specifically constructed so the claimed color matches the board and only identity catches it. Required updating every existing `handleCommand` call site (tests and `WebSocketServer::onMessage`) to actually `join()` a matching-color username first - this wasn't optional test churn, it's what exposed that no test had ever exercised sender identity before. | E1, A3 | 4 new doctest cases (spectator rejected without mutating engine state, unjoined username rejected the same way, a player claiming the opponent's color rejected even though the claim matches the board, each player can command only their own color both directions) + all pre-existing `handleCommand` tests updated to `join()` a legitimate username first. `run_tests.exe` 202/202 (was 198, +4 new). `kungfu_server`/`kungfu_client` rebuilt clean against the new signature. **Live regression check** (this changed an already-live path, not new wiring, so it got one): two real sockets (reused `ws_test_client.py`'s raw primitives to know for certain which side actually got White before sending anything), White's own `e2e4` still lands correctly end-to-end and is logged normally (`move color=w notation=e2e4`) - confirms the identity check doesn't false-reject a legitimate player's own move. The actual rejection paths (spectator/impersonation) can't be exercised live yet - no client can become a spectator or a second connection for the same color until E3 wires rooms - so those stay doctest-only for now, consistent with the original plan's own scope for this task. |
| **E3** ✅ | Wired `client/cli`'s "Room" flow (a post-login menu: `[1] Quick match [2] Create room [3] Join room`, room ID printed on creation) plus E1's own deferred server wiring: `{"type":"create_room"}`/`{"type":"join_room","roomId":...}`, both reaching `GameSession::join()` exactly like matchmaking's `handleMatch()` already does - a new `sendJoinedMessage()` helper builds the `{"type":"joined",...}` response for all three paths (matchmaking, room player, room spectator) so the shape (now including `whiteUsername`/`blackUsername` alongside the original `opponent`) stays one implementation, not three. `ConnectionRegistry`'s `kMaxConnectionsPerSession` raised 2 → 10 (2 seats + up to 8 spectators), uniform across every session as planned. Game-start timing: `createSession()` no longer calls `engine().startGame()` (board-load + lifecycle-start bundled) - it now only `loadBoard()`s, so a room's lone creator still sees the starting position broadcast immediately, but the `onGameLifecycle("start",...)` announcement is deferred to a new `GameEngine::announceStart()` (fires the same event alone, without touching the board), called by `handleMatch()` right after its own two joins (functionally unchanged - both players are already known the instant a matchmaking session exists) and by `handleJoinRoom()` specifically on the join that returns `color == 'b'` (the exact moment a room's roster completes; a later spectator join never re-fires it). | E1, B4 | 2 new doctest cases for `announceStart()` (fires the event without touching the board; `loadBoard()` + `announceStart()` behaves identically to `startGame()`). `run_tests.exe` 204/204 (was 202, +2). `kungfu_server`/`kungfu_client` (CMake, after a reconfigure so the source glob picked up E1's `RoomRegistry.cpp`) rebuilt clean. **Live manual verification, all three paths the user specifically asked for** (server + a real `kungfu_client.exe` for at least one seat per scenario; a persistent raw-socket stub filled the other seat(s) where a short-lived real client's own post-command disconnect would have triggered D4's unrelated 20s auto-resign mid-test): (1) normal flow - creator `create_room`s and gets `{"roomId":"7CZCBN","color":"white",...}` with the starting position broadcasting immediately (creator alone, before anyone else joins); a real client `join_room`s and gets `"You are Black. Playing against E3White."`; `server.log` shows `lifecycle phase=start` firing exactly on that 2nd join, not at creation. (2) Color-impersonation rejection - Black's real connection sends `WPe2e4` (White's own claimed color, and a real white rook is genuinely on that square) → `server.log`: `rejected command: WPe2e4 (ERROR NOT_YOUR_COLOR)`; a follow-up legitimate `BPe7e5` from the same connection lands normally (`move color=b notation=e7e5`), proving the rejection is identity-specific, not a general break. (3) Spectator rejection - a real 3rd client `join_room`s the same room, client prints `"You are spectating: E3White4 (White) vs E3Black4 (Black)."` (confirms the spectator display path too), then sends `WPe2e4` → `server.log`: `rejected command: WPe2e4 (ERROR NOT_A_PLAYER)`, no board change on either remaining connection's next tick, on a still-live (not yet game-over) session. |
...
### Phase G — Instructor gap closures (post-Phase-F, self-audit against the instructor's progress checklist)

Four gaps flagged by an instructor progress-checklist self-audit, not part
of the original deck items A-F. Scope/design for all four proposed and
confirmed before starting (including three explicit open-question
resolutions, folded into each row below); sequencing is deliberate -
**G2 first**, so G3/G4 are born with real socket-test coverage instead of
having it retrofitted, then G3, then G4, then **G1 last**, so the README
documents the final state instead of needing a post-hoc edit.

| Task | What | Depends on | Tests |
|---|---|---|---|
| **G2** ✅ | Real-socket automated tests for `WebSocketServer.cpp` itself (`onOpen`/`onClose`/`onMessage`'s JSON-type dispatch/`broadcastState()`/the tick loop) - the one class in `server/` with zero automated coverage before this, since every other class there is pure logic already dual-compiled and doctested via the root Makefile, while `WebSocketServer.cpp` needs `websocketpp`/Asio directly and so only ever builds under CMake. **Resolved open question: `ctest` integration, not a plain second executable** - cheap to add and builds the "run before commit" habit. Real loopback sockets, not mocks: a new CMake-only test executable (`kungfu_server_tests`, `server/CMakeLists.txt`, mirroring the `SERVER_ONLY_SRC`/`OPENCV_ONLY_SRC` exclusion pattern - it can't join `run_tests.exe`, that build has no `websocketpp`/Asio access) links `WebSocketServer.cpp` + `tests/doctest.h` + a new reusable in-process `websocketpp::client` test helper (`server/tests/WsTestClient.hpp/.cpp`, same open-handler-plus-message-queue-plus-background-`run()`-thread pattern `client/cli/main.cpp` already established, generalized instead of reinvented) and a `ServerFixture` (`server/tests/ServerFixture.hpp`) that runs a real `WebSocketServer` per test on its own port with its own throwaway `data/test_ws_<port>.db`/`.log` (never the real dev database/log). Needed two small, narrowly-justified additions to `WebSocketServer` itself for testability: `stop()` (calls `get_io_service().stop()` - safe to call from another thread, asio's own documented guarantee; abrupt, not graceful, fine for test teardown) and `isListening()` (an `std::atomic<bool>` flipped right after `start_accept()`, so a test harness starting the server on a background thread can poll instead of assuming it's ready) - plus overridable `dbPath`/`logPath` constructor parameters (defaulted to the exact prior hardcoded values, so `server/main.cpp`'s real callsite is unaffected). Real 60s matchmaking / 20s disconnect timeouts actually *firing* are deliberately **not** re-tested here - both are already covered as pure decisions (`MatchmakingTimeout`/`DisconnectTimeout` doctests) and were verified against real wall-clock timers during D3/D4's own manual verification; waiting out 60/20 real seconds per test case here would add real test-suite runtime for no new coverage. Two real bugs found and fixed in the *test* code itself while getting this green (not in `WebSocketServer`): a "drain until idle" helper that never terminates against a still-ticking session (ticks arrive every ~16-30ms, well under any idle-gap threshold short enough to stay fast, so it needs an absolute deadline instead, not an idle-gap one); and a disconnect/reconnect test with the color assignment backwards (matchmaking assigns white to whichever client's `play` reaches the server first - confirmed by the matchmaking test case's own passing assertions - not to whichever variable a test happens to name "white"). | A5-E3 (exercises the full connection lifecycle) | 4 new doctest `TEST_CASE`s under `kungfu_server_tests` (not `run_tests.exe` - see above): login auto-register + wrong-password rejection; matchmaking pairing + a real travel-time move resolving and broadcasting to both sides; room create/join + a 3rd joiner becoming a spectator whose move is silently rejected (`ERROR NOT_A_PLAYER`, board provably unchanged - command rejections are server-log-only, never sent back to the sender, confirmed by reading `WebSocketServer::onMessage`); a closed connection starting the disconnect countdown, broadcast to the opponent, and clearing on reconnect. 46/46 assertions passing, ~2.7s real runtime via `ctest`. `run_tests.exe` reconfirmed unaffected (204/204, matching E3's last recorded count exactly - `make` itself isn't installed on this machine, only the MSYS2 UCRT64 toolchain, so this was verified via a manual `g++`/`gcc` invocation mirroring the Makefile exactly, same as D4's own precedent for this environment). `kungfu_server`/`kungfu_client` rebuilt clean alongside the new `kungfu_server_tests` target. |
| **G3** ✅ | Voluntary `resign` wire command, reusing E2's sender-identity model instead of a claimed-color field: a new literal command string `"resign"` (unambiguous against `GameCommandParser`'s strict-uppercase move/jump grammar), intercepted in `WebSocketServer::onMessage` before `GameCommandParser::parse`, routed through a new `WebSocketServer::handleResign(sessionId, username)` to `GameSession::handleResign(username)` (same `ERROR NOT_A_PLAYER` rejection as `handleCommand` for a spectator/unjoined username, console-logged only like every other command rejection, no response sent to the sender; otherwise `engine().resign(colorOf(username))` - `GameEngine::resign()` already no-ops if `gameOver`, so a stray double-resign is safe for free, no new guard needed). No client-side parsing needed (`client/cli`'s gameplay loop already forwards any typed line verbatim, Task B4's stance) - just a "Type a move, or 'resign' to resign." hint added to the two "you are White/Black" print sites. Logging is automatic for the lifecycle event itself - `GameSession` already subscribes `Logger` to `onGameLifecycle` (A6) - plus one new explicit `resign session=... username=...` log line from `WebSocketServer::handleResign()` for the wire-level event. **Resolved open question: cancel the pending 20s disconnect timer (if any, either seat) for the session when a resign is handled** (new `WebSocketServer::cancelDisconnectTimers(sessionId)`, called on a successful resign) - rather than accepting it as a cosmetic leftover, avoids `whiteDisconnectMs`/`blackDisconnectMs` still counting down in the broadcast for a few ticks after `gameOver` is already true. | A3, D4, E2 | 4 new doctest `TEST_CASE`s in `test_game_session.cpp`: `handleResign` ends the game in favor of the other color both ways (checked via both directions independently), fires the same `lifecycle phase=end` log line a king capture would, rejects a spectator and an unjoined username both with `ERROR NOT_A_PLAYER` and no game-state mutation, and is a safe no-op when called again after the game already ended. `run_tests.exe` 208/208 (was 204, +4 new). 2 new real-socket `TEST_CASE`s in `kungfu_server_tests` (G2): a resign ending the game *and* actually cancelling a live disconnect timer for the other (already-disconnected) seat - the case that actually exercises the resolved open question above, confirmed via `blackDisconnectMs` reading `null` again in the post-resign broadcast, not just `gameOver` flipping true; and a spectator's `"resign"` rejected without ending the game. `kungfu_server_tests` now 6/6 cases, 59/59 assertions. Manual: real two-`kungfu_client.exe` resign exchange against a live `kungfu_server.exe` (White resigns) - `server.log` showed `lifecycle phase=end result=Black Wins` immediately followed by `resign session=0 username=G3Alice`, confirming the EventBus-driven log and the new explicit wire-level log both fire correctly for a real client-initiated resign, not just the synthetic test harness. |
| **G4** ✅ | Push two of the four `EventBus` categories to clients as discrete messages instead of only being inferable a tick later from the periodic snapshot - `onSound` (today reaches **no** networked client at all; A4 deliberately excluded sound from `GameStateSerializer`, same bucket as `captureFlashes`) and `onGameLifecycle` (start/end - meaningful one-shot cues, and this is what makes G3's resign show up immediately rather than only via `gameOver` flipping on the next tick). **Resolved open question: `onSound` + `onGameLifecycle` only, not `onMoveLogged`/`onScoreUpdated`** - both of those are already reflected within one tick via the existing snapshot broadcast, so a discrete push there would shave latency but add no new capability; scope stays to what's genuinely missing today. Same optional-attach pattern as `attachLogger()`: new `GameSession::attachEventSink(std::function<void(const std::string&)>)`, subscribed to `onSound`/`onGameLifecycle` in the constructor (gated non-null, mirrors A6's stance so every existing default-constructed `GameSession()` in tests stays unaffected), publishing a small pre-serialized JSON string built by plain concatenation, not `nlohmann::json` (`{"type":"sound","name":"capture"}`, `{"type":"lifecycle","phase":"end","result":"White Wins"}`) - safe because every field is one of a small fixed set of internal engine-controlled strings, never user input, so there's nothing to escape and no need to pull a JSON library into a class that's had zero JSON dependency until now. `WebSocketServer::createSession()` wires the sink to a lambda that closes over the session id and calls a new `broadcastToSession(sessionId, json)` - the per-connection send loop factored out of `broadcastState()` itself (which now just builds the state string and calls the same helper), so the discrete push and the periodic full-state broadcast share one send loop/try-catch instead of duplicating it. Safe under the confirmed single-threaded `io_context` concurrency model - publishing happens synchronously on the same thread `broadcastState()` already runs on, no new thread-safety surface. **Real ordering bug found and fixed while wiring this in** (not hypothetical - caught by the existing G2 regression suite immediately): `handleMatch()`/`handleJoinRoom()` used to call `engine().announceStart()` *before* `sendJoinedMessage()`, which was invisible before this task (`announceStart()` had no client-facing effect at all pre-G4) but now means the discrete `"lifecycle":"start"` push - which broadcasts to every connection in the session, including the one that just joined - could reach a client *before* that same client's own `"joined"` identity message on the wire, since both are sent on the same connection in call order. Fixed by reordering both call sites to send `"joined"` first, `announceStart()` second - a connection should learn its own identity before a general "the game has started" notification. | A6, D4 (announceStart/resign both need to reach clients promptly) | 6 new doctest `TEST_CASE`s in `test_game_session.cpp`: no sink attached stays silent (mirrors A6's `Logger*` test); a jump publishes exactly one `sound`/`jump` message; a resolved capture publishes both a `move` and a `capture` sound message; a king capture publishes exactly one `lifecycle`/`end` message with the right result; `handleResign` (G3) publishes exactly one `lifecycle`/`end` message; `announceStart()` publishes exactly one `lifecycle`/`start` message with no `result` field. `run_tests.exe` 214/214 (was 208, +6 new). 1 new real-socket `TEST_CASE` in `kungfu_server_tests` (G2): matching pushes a discrete lifecycle-start message (asserted to arrive before any further assumption about ordering breaks), then a real `e2e4`/`d7d5`/`e4d5` capture sequence (the same one B4's own manual verification used) pushes a discrete `sound`/`capture` message to the non-mover's connection too, not just the mover's - confirming the broadcast, not just the publish. `kungfu_server_tests` now 7/7 cases, 64/64 assertions. **Two real bugs found and fixed getting this new test green** (both in the *test*, not the server, and both worth recording since they reveal real mechanics): (1) the ordering bug described above, first caught here as 2 *existing* tests regressing, not a new-test failure; (2) a freshly-landed piece enters a short/long rest cooldown (`RealTimeArbiter`'s `longRestMs_` = up to 800ms) during which a same-piece move is a silent no-op - the test's own "both pawns landed" check passed well before that cooldown had actually elapsed, so the scripted capture move was silently dropped until an explicit extra 1s wait was added after landing. Manual: two real `scripts/ws_test_client.py` raw-socket observers (needed since `client/cli` has no handler for the new `"sound"`/`"lifecycle"` message types - see the open question below) against a live `kungfu_server.exe`, driven through the same real e2e4/d7d5/e4d5 capture sequence - both observers' raw JSON logs showed `{"type":"lifecycle","phase":"start"}` immediately after `"joined"` and `{"type":"sound","name":"capture"}` for the real capture, confirmed via `server.log`'s own `score`/`sound name=capture` lines at the same real timestamp. One verification-script gotcha, not a server bug: `ws_test_client.py` only flushes its queued incoming messages right before each `input()` call (documented in its own header) - a message arriving after the last scripted line is sent sits queued and is silently dropped when the script hits EOF with no further `input()` call to flush it before; fixed by adding one trailing blank line to force a final flush, not by changing the server. |
| **Open question (not yet resolved): should `client/cli` actually render the new `"sound"`/`"lifecycle"` messages?** G4 only builds and verifies the server-side push - `client/cli`'s `handleServerMessage()` has no branch for `"sound"`/`"lifecycle"` at all, so they're silently ignored by the one existing shell client today (confirmed during G4's own manual verification, which had to fall back to `ws_test_client.py` instead). Nothing in the original deck or this Phase G self-audit asked for shell-client sound/UI, and `client/cli` has no audio output at all today (`miniaudio`/`SoundManager` are graphics-build-only) - so this is flagged as a real gap but deliberately left unscoped rather than guessed at. |
| **G1** ✅ | Root `README.md` - none existed before this, build/run instructions for all four binaries lived only as prose scattered across `CLAUDE.md`, `kungfu-graphics/cpp/README.md`, and this file's own manual-verification sections. Synthesis, not new research, but every command in it was re-verified rather than copied blind: `run_tests.exe` (`make && ./run_tests.exe`, plus the MSYS2-UCRT64-shell-not-PowerShell caveat A5 already established and the manual-`g++`-invocation fallback D4/G2/G3/G4 all actually needed on this machine); the graphics `KungFuChess.exe` (links to `kungfu-graphics/cpp/README.md` for the `OpenCV_451` download, VS Code + CMake Tools per `.vscode/settings.json`, MSVC-not-MinGW noted with the reason why, and the `KUNGFU_ASSETS_ROOT`/`KUNGFU_SOUNDS_ROOT` absolute-path defines confirmed by reading `kungfu-graphics/cpp/CMakeLists.txt` directly - meaning unlike the other three binaries, this one doesn't need to be run from the repo root); `kungfu_server.exe`/`kungfu_client.exe` (MSYS2/ucrt64 `cmake -S server -B server/build -G Ninja` + `cmake --build server/build`, run from the repo root, port `9002` confirmed from `server/main.cpp`, `kungfu_client.exe`'s optional URI argument confirmed from its own `argc`/`argv` handling); the new `ctest --test-dir server/build --output-on-failure` invocation (G2) actually run to confirm the flag/output shape before writing it down; plus a pointer to `scripts/build_vpl_submission.sh` for the course grader zip and a link into `CLAUDE.md`/`docs/tasks/` for the full design history. Deliberately last in Phase G so it documents G2/G3/G4's real final state. | G2, G3, G4 (documents their end state) | N/A - documentation only. Every command shown was actually executed during this task (not just copied from other docs) to confirm it works as written. |
