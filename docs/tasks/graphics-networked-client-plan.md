...
> **Status: paused before implementation.** This is a reviewed-but-not-started
> plan — written up for reference so work can resume here later, not a log of
> completed work like `server-phase-plan.md`. Nothing in this file has been
> built yet. Pick up at **H1** when resuming.
...
|---|---|---|---|
| **H1** | Get `websocketpp` + Asio + `nlohmann` building inside `kungfu-graphics/cpp/CMakeLists.txt` under **MSVC** (not MinGW/g++, unlike `server/`). Pin the same versions the server already validated (Asio `1.18.2`, websocketpp `0.8.2`) via `FetchContent`; reference `third_party/nlohmann` directly the same way `server/CMakeLists.txt` does. **This is the real open risk in this whole plan** — Phase A of `server-phase-plan.md` chose MinGW specifically to avoid ever having to prove websocketpp under MSVC, and that proof has never happened in this repo. Per decision 5: if it doesn't compile clean, **stop and report back**, don't improvise a fallback. | — | Manual: a trivial connect-and-print-"connected" smoke test against a running `kungfu_server.exe`, built via the existing VS Code + CMake Tools flow. |
...
| **H3a** | Safety-first refactor, zero behavior change: extract today's entire local-play loop (`BoardParser` → `GameEngine` → `Controller` → `BoardView`/`HudView` → `EventBus` wiring) out of `main()` into its own function (e.g. `runLocalGame()`), then add a new top-level **"Local Play" / "Online Play"** mode-select screen (`Img`-only) in front of it. Selecting "Local Play" calls `runLocalGame()` unchanged. No networking dependency at all — can be done and verified independently of H1/H2. | — | Manual: existing local-play headless-PNG-probe / click-through verification re-run and confirmed identical to pre-refactor behavior (this task must introduce zero observable change to local play). |
...
Next step when picked back up: **H1**. Nothing else in this file has
external dependencies that block starting H1 or H3a in parallel, but H2/H4/
H5/H6/H7 all sit behind H1 actually working.
