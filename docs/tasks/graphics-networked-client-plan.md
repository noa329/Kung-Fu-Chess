...
> **Status: in progress, paused after H1.** This started as a
> reviewed-but-not-started reference plan; **H1 is now done** (see its row
> below), proving the schedule-critical MSVC risk resolves cleanly. Paused
> again here to return to server-scaling design work. Pick up at **H2**
> (or **H3a**, which has no dependency on H1/H2) when resuming.
...
or **H3a** (the zero-risk local-play-extraction + mode-select refactor,
which has no dependency on H1/H2 and can be done in parallel/first). H1 is
done — the MSVC risk that gated everything else in this plan is resolved,
so H4/H5/H6/H7 are no longer blocked on an open risk, just on the
