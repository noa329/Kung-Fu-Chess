...
> **Status: in progress, paused after H2.** This started as a
> reviewed-but-not-started reference plan; **H1, H3a, and H2 are now done**
> (see their rows below) — the schedule-critical MSVC risk is resolved, the
> local-play path is cleanly isolated behind a mode-select screen with zero
> behavior change, and the wire↔`GameSnapshot`/event translation layer is
> built and doctest-covered with no UI dependency at all. **H3a's manual
> verification still has one open gap**, unrelated to H2: the mode-select
> screen's rendering is confirmed via a real screenshot, but a live
> click-through into `runLocalGame()` couldn't be confirmed by automation
> (desktop-automation flakiness, not a suspected code issue) — the user is
> doing that real click-through manually. Pick up at **H4** (networked click
> handling) or **H3b** (real Online Play screens) when resuming — both only
> depend on H1/H2, which are done.
...
Next step when picked back up: **H4** (networked click handling, builds
directly on H2's translator) or **H3b** (the real Online Play login/menu/
room screens, replacing H3a's "coming soon" stub) — both only depend on
H1/H2, which are done. H3a's one open verification gap (a real
click-through into `runLocalGame()`) is being closed manually by the user,
outside this task sequence.

H1, H3a, and H2 are all done — the MSVC risk that gated everything else in
this plan is resolved, local play is cleanly separated from where the
online path will land, and the wire-format translation layer both H4 and
H5 need already exists and is tested. Remaining tasks (H3b, H4, H5, H6, H7)
are blocked only on task order now, not on any open risk.
