...
## שלב 5 (ביקורת): אימות בפועל, לא רק הנחה שהכל כבר עובד

**הרקע:** אחרי שלב 7, המשתמשת דיווחה על באג בפועל - כלי שנוחת אחרי
מהלך לא נראה נכנס ל-`long_rest` ויזואלית ("נראה כמו idle"), אף
שבדיקת ה-cooldown עצמה (engine-level) כן חוסמת מהלך נוסף. זה בדיוק
המקרה שה-doctest suite לא יכול לתפוס - `BoardView`/`PieceAnimator`
תלויים ב-OpenCV ולכן לא רצים ב-Makefile הרגיל.

**מה נבדק בפועל:** נוסף accessor קטן ל-`BoardView`
(`debugAnimatorState(r, c)` - קריאה בלבד, מחזיר את המצב האמיתי של
ה-animator שמטמון אצל תא נתון) כדי לבדוק את המצב **בפועל**, לא להסיק
מפיקסלים מצוירים. הרצתי לולאה חד-פעמית שמדמה בדיוק את `main.cpp`
(צעדי `wait(16ms)` קטנים, `syncFromSnapshot`+`update` כל פריים) ווידאתי:
`GameEngine::snapshot().cellStates` עובר ל-`"long_rest"` בדיוק בפריים
ההגעה, ו-`BoardView`'s ה-animator עובר לאותו מצב **באותו פריים** ונשאר
שם יציב (בלי ריצוד/חזרה ל-idle) עד שה-cooldown נגמר - שני הצדדים
מתעדכנים יחד. כלומר `syncFromSnapshot` **כן** קוראת נכון את
`cellStates` ומזרימה אותה נכון ל-animator הנכון - שום קוד שבור.

**הממצא האמיתי:** `pieces2/PW/states/long_rest/sprites/{1,3,5}.png`
נראים כמעט זהים ויזואלית ל-`idle` (קבצים שונים לפי hash, אבל בלי
שינוי צבע/תנוחה נראה לעין) - בניגוד ל-`pieces1` שלפחות צובע טקסט
debug על הספרייט. כלומר זו מגבלת אמנות-placeholder, לא באג קוד.
נמצא גם אגב כך אי-סימטריה בין הסטים: `pieces1`'s `long_rest` הוא
`is_loop: false` (one-shot, חוזר ל-idle לבד כשמסיים), בעוד `pieces2`'s
הוא `is_loop: true` (לולאה עד שה-snapshot אומר אחרת) - שניהם תקינים,
אבל לא לזהות בהנחה שהתנהגות הזמנים זהה בין הסטים.

**החלטה:** השארתי את `debugAnimatorState()` ב-`BoardView` (במקום
להסיר אחרי האבחון) - זה seam קטן, read-only, בדיוק מהסוג שה"testing
convention" של הפרויקט כבר דורש עבור שינויי רינדור (בדיקה headless
מול הרצה אמיתית ולא רק הבטה בפיקסלים).

**לקח כללי לפרויקט:** "עבד בבדיקות מבודדות" (doctest, headless probe
עם `wait()` אחד גדול) לא הוכיח "עובד במשחק חי" - רק סימולציה מדויקת
של הלולאה האמיתית (צעדי זמן קטנים, סדר הקריאות המדויק) תפסה את
ההבדל בין "יש state נכון ב-snapshot" ל"רואים את זה על המסך".
